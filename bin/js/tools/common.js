                ...
                if (instance.workDone) {
                    ...
                    while (instance.buffer.length > 0) {
                        ...
                        if ((instance.totalLen >= 5) && (instance.msgCache.byteLength + msgLen >= instance.totalLen)) {
                            ...
                            var left = null;
                            if (instance.msgCache.byteLength + msgLen > instance.totalLen) {
...
var sendToOnline = function (cmd, data) {
...
function showjiazai(shijian) {
    if (shijian === void 0) { shijian = null; }
...
var recvOnlineData = function (cmd, data) {
    ...
    if (cmd == protocol.user_login) {
        ...
        if (Global.share_callback) {
            if (Global.share_callback["key"] == "new_player") {
        ...
        if (window["WxModular"]) {
            if (wx) {
                wx["postMessage"]({
                    type: 1,
                    data: {
                        score: Global.UserData.getUserData("shangjing").remain_money.toFixed(2),
                    } // 	
                });
                wx["postMessage"]({
                    type: 5,
    ...
    else if (cmd == protocol.user_hudong_sjddz_hall) {
    ...
    else if (cmd == protocol.user_update_fuhuoka) {
    ...
    else if (cmd == protocol.user_gold_compensate) {
        var time = 2;
...
function runScene(scene, arg, reset) {
...
function httpPost(query, args, callback, target, params, isSync) {
    if (params === void 0) { params = null; }
    if (isSync === void 0) { isSync = null; }
...
function addClick(btn, func, caller, removeAll, args) {
...
// 单次点击
function addOnceClick(btn, func, caller) {
    if (caller === void 0) { caller = null; }
    btn.offAll();
    btn.on(Laya.Event.CLICK, caller, func);
}
//批量清除点击事件
var deleClick = function (btnGround, func, caller) {
    if (caller === void 0) { caller = null; }
    var _ = function (target) { target.off(Laya.Event.CLICK, caller, func); };
    if (Array.isArray(btnGround))
        for (var i = btnGround.length; i--;) {
            _(btnGround[i]);
        }
    else {
        _(btnGround);
...
function addSelect(box, func, caller) {
    if (caller === void 0) { caller = null; }
...
// }
// 特殊打印
var _log = function (text, font, color) {
    if (font === void 0) { font = 30; }
    if (color === void 0) { color = '#cc3535'; }
    try {
        console.log("%c" + JSON.stringify(text), "font-size:" + font + "px;color:" + color + ";font-family:fantasy;word-wrap:break-word;");
    }
    catch (err) {
        console.log(text);
        console.error('打印对象有错误');
    }
};
function getPara(key, url) {
...
function setWxInfo(data) {
    ...
    Global.YfSDK["showShare"](data.title || para[data.kind].title, data.desc || para[data.kind].desc, "http://wawa.yunfanshidai.com/h5game/icon.png");
    //    if(data.share == 1){
...
function showGoldshare_gold(data) {
    if (data === void 0) { data = null; }
...
function get_gold_and_resurrection(data, type) {
    var action_gain = new ui.sjddz.action_gain_goldUI();
    if (type == "gold") {
        ...
        share: function () {
            wx["request"]({
                ...
                success: function (res) {
                    ...
                    // 主动转发
                    window["shareBTN"] = function (query) {
                        wx.shareAppMessage({
                            title: res.data.info,
