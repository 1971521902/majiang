module games{
    export class ddz_sjGameScene{
        private ddz_sjgameUI:Hallddz;
        main:ui.sjddz.ddz_sjgameUI;
        mlayer:ui.sjddz.fileApplication_sjddzUI;
        loading_jia:any={};
        data:Object;
        selfPos:number;
        start:boolean;  //游戏是否开始
        tempDlg:Laya.View; //唯一弹窗
        RuleCheck:Object;
        players:Object;
        playerNum:number;
        zhuang_img:null;
        ...
        lastidx:number;
        autoindex:number;
        canCardData:Array<any>;
        exitRoomRole:number;
        shangjing_ju:number;
        protQueue:Array<Object>;
        loadEnd:boolean;
        gold_changci:boolean;
        private __Login ={};
        constructor(data){
            this.data = data;
            this.players = {};
        ...
        private onLoaded():void{
            ...
            if(this.data["room_state"]){
                ...
                if(this.data["room_state"] == STATE.ROOM_WAITING_RESTART){  //结算
        ...
        private up_goldbeishu(num){            //  金币场  倍数
            var self = this
            if(self.data["tbl"].game_type == "gold"){
        ...
        private updateState(){
            ...
            for(var i=1;i<=3;i++){
                ...
                addClick(seekByName(panel, "btn_" + i) || seekByName(panel, "chupai") || seekByName(panel, "buchu")  , function(e){
                    if(this.name == "btn_1" || this.name == "buchu" ){               //  不出
                        self.hideBtn()
                    ...
                    }else if(this.name == "btn_2"  || this.name == "chupai"){         //  出牌
        ...
        private showEffect(passcard){      // 音乐
            var self = this
            var obj 
        ...
        private voice(){
            ...
            voice_list.renderHandler = new Handler(this, function(cell:Box, index:number){
                var msgText = (cell.getChildByName("msgText") as Laya.Text);
                msgText.text = cell.dataSource["DESC"];
            });
            voice_list.mouseHandler = new Handler(this, function(e:Event, index:number){
        ...
        private showUserInfo_shu(info){
            ...
            panle_skill.renderHandler = new Handler(this,(cell:Box,index:number):void=>{
                console.log(132456)
        ...
        private showReadyStatus(data):void{
            var self = this
        ...
        private getPlayerPos(pos):number{
            var self = this
        ...
        public recvData(cmd, data){
            ...
            }else if(cmd == protocol.user_get_money){         // 开红包
                var time = 2
            ...
            }else if(cmd == protocol.user_hudong_biaoqing){        // 互动的 1315
                self.action_type(data)
            }else if(cmd == protocol.user_room_state){
                this.roomState = data   
            }else if(cmd == protocol.user_card_qiang_di_zhu){     //  叫分 3000
            ...
            }else if(cmd == protocol.user_card_jia_bei){     //  加倍 3001
            ...
            }else if(cmd == protocol.user_card_passcard || cmd == protocol.user_card_pass ){     //  出牌607
            ...
            }else if(cmd == protocol.user_update_socre){     //  刷新分数
            ...
            }else if(cmd == protocol.user_gold_compensate_get){     // 破产 刷新分数 2006
            ...
            }else if(cmd == protocol.user_sjddz_share_fuhuoka){     // 失败分享群获得复活卡 4003
                var pan1 = this.main["player1"]
                var aaa = seekByName(pan1, "cardValue").text
                seekByName(pan1, "cardValue").text = Number(seekByName(pan1, "cardValue").text) + data
                this.gold_changci = true
            }else if(cmd == protocol.user_con_resurrection){     // 消耗一张复活卡1162
                showTip("复活成功")
            }else if(cmd == protocol.user_single_result){     //  小结算
                if(data.pooker_list){
            ...
            }else if(cmd == protocol.user_final_result){     //  大结算
            ...
            }else if(cmd == protocol.user_disagree_exit){     //  申请失败
                if(this.mlayer) {
                    this.mlayer.removeSelf();
                    this.mlayer = null
            ...
            }else if(cmd == protocol.user_update_exit_room){     //  625  解散房间
                if(this.players[data-1].id == Global.UserData.getUserData("id") ){
                    return
                }
                this.exitGameingShow(data-1,null)
            }else if(cmd == protocol.user_update_yes_exit_room){     //  626   选择同意不同意
            ...
            }else if(cmd == protocol.user_send_voice){        //  动画语音
                 seekByName(self.main,"voice_bg").visible = false
                  var cur = data.idx;
                   var sign:Laya.Button = this.getHeadSign(cur);
        ...
        private getHeadSign(data, voice=false){
          if(this.start){
        ...
        private showResult(data,final){
            ...
            if(final){
                ...
                if(is_gold == "shangjing" ){              //  赏金场     shangjing
                    ...
                    if(is_win ==  0 ){               //   流局
                        ...
                        addClick(seekByName(show_shibai, "share_flock"), function(){                             // 赏金场的继续游戏
                            ...
                            if(seekByName(show_shibai, "free_fuhuo")){
                                ...
                                addClick(seekByName(show_shibai, "free_fuhuo"), function(){                             //  分享群后复活
                        ...
                        addClick(seekByName(show_shibai, "fanhui"), function(){                                 // 赏金场的继续游戏
                    ...
                    addClick(result.continue_game, function(){                             // 赏金场的继续游戏
                        if(is_win < 0 ){       // 输了之后
                    ...
                    if(data[self.selfPos+1] && data[self.selfPos+1].win ==  0 ){           // 流局
                ...
                addClick(seekByName(result, "confirmBtn"), function(e){
                    if( is_gold == "gold" ){                     
                        runScene(gameView.hall_sjddz);
                        var room_type
                        if(self.gold_changci){          //  金币补助  写死初级场  type7
                            room_type  =  7   
                        }else{
        ...
        private startGame(data, reconnect):void{
            var pos 
            var self = this;
            self.start = true
            ...
            if(self.roomState==STATE.ROOM_WAITING_JIAOFEN && jiao_fen_list){                           
            ...
            if(self.roomState==STATE.ROOM_WAITING_JIABEI && jia_bei_list){
                if (jia_bei_list[self.selfPos] == true && self.curIndex -1 == self.selfPos){
                    this.showJiabei(true)
                }
                this.createbuchuTimer(self.curIndex,5)          // 加分和加倍    默认5秒
            }
            if  (self.roomState!=STATE.ROOM_WAITING_JIAOFEN ){
            ...
            if(self.roomState==STATE.ROOM_WAITING_PUTOP){                                                // 出牌 10
                if (self.curIndex -1 == self.selfPos){
                    if(reconnect && reconnect["put_card_list"]){
                    ...
                    if(time<0){
                        ...
                        if(self.data["tbl"].game_type != null){
                            for (var i = 1; i <=3; i++){
                                ...
                                let zhuang = seekByName(self.main, "player" + i)
        ...
        private getFrontIndex(curindex):number{   // 获取上家出牌玩家的  idx
        ...
        private checkPassCard(last,passtime){    //检测可以出的牌
            var self = this
            var  lastcard=last 
            if (lastcard){
        ...
        private chupaiStatus(canCardData,passtime){             // 出牌状态
            var self = this
            this.hideBtn()
            self.players[self.selfPos].backAllPoker()
            ...
            if (canCardData && typeof(canCardData)=="object" && canCardData.length==0){          // 不出
                    seekByName(panel, "btn_1").visible = false
                    seekByName(panel, "btn_2").visible = false
                    seekByName(panel, "btn_3").visible = false
        ...
        private showJiaofen(bol,xianzhi){
            var self = this
            this.hideBtn()
            var opt = seekByName(this.main, "opt")
        ...
        private createbuchuTimer(idx,temp_tim = null){
            var time = 10
            if(temp_tim){
                time = temp_tim
            }
        ...
        //申请解散
        private exitGameingShow(data,info):void{
            let self = this
        ...
        public updateUI(key, data){
            if(data.roomid > 0 ){ //有房号且未开始,不是房主    // && not self.start   && not self:isManager() 
            ...
            }else if(key == "relive_card_num"){              //复活卡    gold_changci  复活卡没有赢的
                console.log("复活卡")
                if( data && data > 0){                      
